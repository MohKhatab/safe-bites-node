...
const getCart = async (req, res, next) => {
  try {
    ...
    res
      .status(200)
      .send({ message: "Cart recieved successfully", data: cart.products });
  } catch (err) {
    next(err);
  }
};
...
const updateCart = async (req, res, next) => {
  try {
    ...
    const indexItem = cart.products.findIndex(
      (item) => item.productId.toString() == productId
    );
    if (indexItem > -1) {
      if (quantity > 0) {
...
const removeFromCart = async (req, res, next) => {
  try {
    ...
    // if cart products length don't change that means the product doesn't exist in the cart
    if (cartLength == cart.products.length) {
